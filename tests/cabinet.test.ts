/*
 * Vendored from obsidian-importer `tests/onenote-file/cabinet.test.ts`
 * (MIT, Copyright (c) 2023 Obsidian), with import paths and __dirname adjusted.
 *
 * These are oracle tests: each .cab was produced by the Windows `makecab`
 * utility, so expanding one and getting the source bytes back proves the LZX
 * port agrees with Microsoft's compressor rather than with its own output.
 */
...
import { readCabinet, readCabinetIndex } from '../src/onenote-file/cabinet/cabinet';
